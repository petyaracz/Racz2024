---
title: 'Supplementary Information. Lexical and social effects on the learning and integration of inflectional morphology.'
author: "Rácz, Péter"
date: "`r Sys.Date()`"
...
b = read_tsv('~/Github/published/Racz2024/exp_data/baseline/baseline_tidy_proc.tsv')
bd = read_tsv('~/Github/published/Racz2024/exp_data/baseline/baseline_tidy.tsv')
st = read_tsv('~/Github/published/Racz2024/resource/exp_input_files/esp/esp_master_input.tsv')
d = read_tsv('~/Github/published/Racz2024/exp_data/esp/esp_master_all_filtered.tsv')
allduf = read_tsv('~/Github/published/Racz2024/exp_data/esp/esp_master_all_unfiltered.tsv')
alldf = read_tsv('~/Github/published/Racz2024/exp_data/esp/esp_master_all.tsv')

iuf = read_tsv('~/Github/published/Racz2024/exp_data/esp/esp_master_sleep_pt1_pt2_all_unfiltered.tsv')
i = read_tsv('~/Github/published/Racz2024/exp_data/esp/esp_sleep_pt1_pt2_master_all_filtered.tsv')

levelling = read_tsv('~/Github/published/Racz2024/resource/real_words/ik_verbs/ikes_pairs_webcorpus2.tsv')
voweldeletion = read_tsv('~/Github/published/Racz2024/resource/real_words/epenthetic_stems/epenthesis_pairs_webcorpus2.tsv')
frontharmony = read_tsv('~/Github/published/Racz2024/resource/real_words/front_harmony/fh_pairs_webcorpus2.tsv')

esp_table = read_tsv('~/Github/published/Racz2024/analysis/esp_analysis/esp_fit.tsv')
posttest_table = read_tsv('~/Github/published/Racz2024/analysis/esp_analysis/posttest_fit.tsv')
esp2_table = read_tsv('~/Github/published/Racz2024/analysis/esp_analysis/sleep_analysis/esp2_fit.tsv')
posttest2_table = read_tsv('~/Github/published/Racz2024/analysis/esp_analysis/sleep_analysis/posttest2_fit.tsv')
...
Here, we give descriptions and report on the baseline experiment for three variable patterns, levelling, vowel deletion, and front harmony. __We report on the main experiment and the integration experiment and provide a discussion for two of these patterns, levelling and vowel deletion.__
...
Participants completed the task in a median `r round(b6,2)` minutes. Given the randomisation, the number of responses per nonce word was not the same. Participants gave `r b4[1]`--`r b4[2]` responses to each nonce form, with a median of `r b5`. The three stimulus lists did not differ significantly in preference for variant 1 / 2 (based on a binomial GLMM predicting probability of variant 1 from list number with participant and target word grouping factors).
...
`r d1` participants completed the main experiment in late 2022 and early 2023. Sample size was pre-set to 7 per unique list (12 lists) or 21 per condition (high / low x typical / reversed x levelling / vowel deletion). `r d1-d1c `participants who matched our exclusion criteria (see below) were excluded. Then, we kept data from the first seven participants in each list (excluding an additional `r d1c-d1b` participants), resulting in a final total of 7 x 3 x 2 x 2 x 2 = `r d1b` participants, `r d2` women. Median age was `r round(d3,2)`. Participants completed the task for course credit.
...
`r 18144-nrow(d)` trials or `r round((1-(nrow(d)/18144))*100,2)`% of all trials were dropped. This is a relatively high number. This is likely both because the task was difficult and because participants completed it in an uncontrolled environment, both contributing to higher variability. Participants completed the matching game and the post-test in 10-15 minutes.
...
`r i1b` participants completed the task in Summer 2023 on [Prolific](https://www.prolific.co/). They were all native speakers of Hungarian, residing in Hungary, age-matched to the sample of the main experiment. `r i2` were women. The median age was `r round(i2,2)`. Participants received 4 GBP for completing the task, 2 GBP for part I and 2 GBP for part two. No participants met the exclusion criteria.
